...
for i in range(0,logs_range):
	...
	if end_unit==1: #if end_unit=1 then we have partiotned the graph so initialize the eventUnit and end_unit
		...
		temp_event_list = []
		if lms_pid in eventUnit:
			for it in range(1,len(eventUnit[lms_pid])+1):
				if isinstance(eventUnit[lms_pid][-it],str):
					temp_event_list = eventUnit[lms_pid][0:len(eventUnit[lms_pid])+1-it]
					break
				else:
					temp_list.append(eventUnit[lms_pid][-it])

			if(len(temp_event_list)!=0):
				G.append(temp_event_list)
			del eventUnit[lms_pid]
		if lms_pid not in eventUnit:
			eventUnit[lms_pid] = []

		for it in range(1,len(temp_list)+1):
			eventUnit[lms_pid].append(temp_list[-it])	
		eventUnit[lms_pid].append(data[i]["lms"])
		end_unit = 0
		
	else: #if not end_unit then add the log event to its corresponding pid
		# if "data" in data[i]:
		if "pid" in data[i] and "lms" in data[i]:
			lms_pid = str(data[i]["pid"]).strip()
			if lms_pid not in eventUnit:
				eventUnit[lms_pid] = []
			eventUnit[lms_pid].append(data[i]["lms"])

		if "pid" in data[i] and "lms" not in data[i]:
			lms_pid = str(data[i]["pid"]).strip()
			if lms_pid not in eventUnit:
				eventUnit[lms_pid] = []
...
# new_G = []
# [new_G.append(x) for x in G if x not in G]
	...
	for log in execution_unit:
		if "exe" in log:
			...
			if "syscall_name" in log:
				if (str(log["syscall_name"]) in ["openat","open","creat","unlink","unlinkat","execve"]) or str(log["syscall_id"]) in ["257","2","85","87","263","59"]:
